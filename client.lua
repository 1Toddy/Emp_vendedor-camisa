...
emP = Tunnel.getInterface("emp_vendedor-camisa")
job = Tunnel.getInterface("emp_vendedor-camisa")
			...
			if not servico then
				if distance < 1 then
					if IsControlJustPressed(0, 38) then
						...
						servico = true
						locais = 1
						CriandoBlip(vendedor-camisa,locais)
					end
				end
			end
		end
...
Citizen.CreateThread(function()
	while true do
		local 1Toddy = 1000
	    if servico then 
		    local x,y,z = 132.22,-3187.67,5.86
		    local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()),x,y,z,true)
		    if distance <= 5 then
		        1Toddy = 3
...
Citizen.CreateThread(function()
	while true do
		...
		if servico then
			...
			local bowz,cdz = GetGroundZFor_3dCoord(vendedor-camisa[locais].x,vendedor-camisa[locais].y,vendedor-camisa[locais].z)
			local distance = GetDistanceBetweenCoords(vendedor-camisa[locais].x,vendedor-camisa[locais].y,cdz,x,y,z,true)
			if distance < 10 then
				1Toddy = 1
					...
					if distance < 2 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if animacao then
								...
								emP.checkPayment()
								animacao = false
								if locais == #vendedor-camisa then
									locais = 1
								else
...
-- FUNCÕES
-----------------------------------------------------------------------------------------------------------------------------------------
function CriandoBlip(vendedor-camisa,locais)
	blips = AddBlipForCoord(vendedor-camisa[locais].x,vendedor-camisa[locais].y,vendedor-camisa[locais].z)
...
function FadeRoupa(time,tipo,idle_copy)
	...
	if tipo == 1 then 
		...
		TriggerServerEvent("emp_vendedor-camisa:roupa")
	end
	DoScreenFadeIn(800)
end
